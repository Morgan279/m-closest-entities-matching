...
public class Filter {
    ...
    public boolean checkCondition(Match curMatch, int level, Vertex v) {
        ...
        return true;
        //return LDF(matchingOrder[level], v) && edgeCheck(curMatch, level, v);
    }
    ...
    public boolean edgeCheck(Match curMatch, int level, Vertex v) {
        Vertex u = matchingOrder[level];

        for (int i = 0; i < level; ++i) {
            for (Vertex uNeighbor : queryGraph.getNeighbors(u)) {
                if (uNeighbor.equals(matchingOrder[i]) && !dataGraph.hasEdge(v, curMatch.get(uNeighbor))) {
                    return false;
                }
            }
        }
