...
public class ExactScanAlgorithm implements MClosestEntitiesMatchingAlgorithm {
    ...
    public MatchGroup query(Graph dataGraph, List<Graph> queryGraphs) {
        ...
        double nonSpatialLevelCostTime = (endTime - startTime) / 1e6;
        ...
        if (feasibleGroup.diameter == 0) {
            if (GlobalConfig.TIME_RATIO_TEST) {
                double spatialLevelCostTime = (System.nanoTime() - startTime) / 1e6;
                GlobalData.timeRatio = nonSpatialLevelCostTime / (spatialLevelCostTime + nonSpatialLevelCostTime);
            }
